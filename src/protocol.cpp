...
#define BUFLEN_SOCK 100  //Max length of buffer
...
public:
    ...
    void createSocket() {
         if((mi_sock = socket(AF_INET , SOCK_DGRAM , 0 )) == INVALID_SOCKET)
    {
    ...
    void bindSocket() {
          if( bind(mi_sock ,(struct sockaddr *)&server , sizeof(server)) == SOCKET_ERROR)
    {
        printf("Bind failed with error code : %d" , WSAGetLastError());
        exit(EXIT_FAILURE);
    } }
    void responder(const char *mensaje) {
      if (sendto(mi_sock, mensaje, strlen(mensaje) , 0 , (struct sockaddr *) &si_other, slen) == SOCKET_ERROR)
        {
            printf("sendto() failed with error code : %d" , WSAGetLastError());
            exit(EXIT_FAILURE);
        }
    }
    std::string escucharSocket() {
     printf("Esperando FEN por el Socket...\n");
        fflush(stdout);
        memset(buf,'\0', BUFLEN_SOCK);
        //try to receive some data, this is a blocking call
        if ((recv_len = recvfrom(mi_sock, buf, BUFLEN_SOCK, 0, (struct sockaddr *) &si_other, &slen)) == SOCKET_ERROR)
        {
...
void loop() {
 ...
 init();
   ...
   Delay = false;
   search_clear();
  // std::string mi_pos = "position fen 2Q5/1p2rpp1/p5kp/8/P1Bqb3/8/1PP3PP/5R1K w - - 0 1";

   std::string mi_pos = miServerSock.escucharSocket();
   std::string a_go = "go movetime 100";

   parse_position(const_cast<char*>(mi_pos.c_str())); // tu pusiste estas lineas mike
   parse_go(const_cast<char*>(a_go.c_str()));

miServerSock.responder(mejor_jugada);

   ASSERT(!Searching);
   ASSERT(!Delay);
    }

    miServerSock.cerrarSocket();
...
static void send_best_move() {
   ...
   if (pv[0] == move && move_is_ok(pv[1])) {
      ...
      strcpy(mejor_jugada, move_string);
      //move_string es lo que quieres enviar: "c4f7"
   } else {
      strcpy(mejor_jugada, move_string);
      printf("Mike not ok: %s\n", move_string);
   }
}
